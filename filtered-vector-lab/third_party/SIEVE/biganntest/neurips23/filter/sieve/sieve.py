...
class Sieve(BaseFilterANN):

    def __init__(self, metric, index_params):
        self.historical_filters_file = index_params.get('historical_filters_file', '')
        self.historical_filters_percentage = float(index_params.get('historical_filters_percentage', 0.25))
        self.is_and = ast.literal_eval(index_params.get('is_and', 'True'))
        self.M = int(index_params.get('M', 16))
        self.ef_construction = int(index_params.get('ef_construction', 40))
        self.index_budget = float(index_params.get('index_budget', 2.0))
        self.bitvector_cutoff = int(index_params.get('bitvector_cutoff', 1000))
        self.workload_window_size = int(index_params.get('workload_window_size', 100000))
        self.heterogeneous_indexing = ast.literal_eval(index_params.get('heterogeneous_indexing', 'True'))
        self.heterogeneous_search = ast.literal_eval(index_params.get('heterogeneous_search', 'True'))
        self.num_index_construction_threads = int(index_params.get('num_index_construction_threads', 1))
        self.ef_search = 10
        self.is_range = False
    ...
    def filtered_query(self, X, filter, k):
        ...
        if not self.is_range:
            ...
            # Uncomment for binned recalls/QPSes
            ...
            search_start = time.time()
            nq = X.shape[0]
            self.res, _, _ = self.index.batch_filter_search(
                X,
                filters,
                nq,
                k,
