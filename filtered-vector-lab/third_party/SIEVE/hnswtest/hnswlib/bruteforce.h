...
class BruteforceSearch : public AlgorithmInterface<dist_t> {
 public:
    ...
    std::priority_queue<std::pair<dist_t, labeltype >>
    searchKnnAcorn(const void *query_data, size_t k, BaseFilterFunctor* isIdAllowed = nullptr) const {
        assert(k <= cur_element_count);
        std::priority_queue<std::pair<dist_t, labeltype >> topResults;
        if (cur_element_count == 0) return topResults;
        for (int i = 0; i < k; i++) {
    ...
    std::priority_queue<std::pair<dist_t, labeltype >>
    searchKnnInPlaceFilter(const void *query_data, size_t k, BaseFilterFunctor* isIdAllowed = nullptr) const {
        assert(k <= cur_element_count);
        std::priority_queue<std::pair<dist_t, labeltype >> topResults;
        if (cur_element_count == 0) return topResults;
        for (int i = 0; i < k; i++) {
