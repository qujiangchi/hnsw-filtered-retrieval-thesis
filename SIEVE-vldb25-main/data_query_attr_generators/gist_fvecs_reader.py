...
if __name__ == "__main__":
    query_matrix = fvecs_read("biganntest/data/gist/gist_query.fvecs", np.int32)
    dataset = fvecs_read("biganntest/data/gist/gist_base.fvecs", np.int32)
    ...
    pickle.dump(np.transpose(data_filters_matrix), open("biganntest/data/gist/gist_data_attrs.pkl", "wb"))
    ...
    while queries_generated < total_queries:
        # Generate a sorted 4-element template from normal distribution
        template = sorted(np.random.normal(loc=0, scale=0.12, size=4))
        # Randomly permute the template
        if random.random() > 0.5:
            template.insert(1, template.pop(3))
        else:
            template.insert(1, template.pop(2))
        if random.random() > 0.5:
    ...
    for template, count in templates.items():
        ...
        if max(f0_left, f1_left) <= min(f0_right, f1_right):
            selectivity = max(f0_right, f1_right) - min(f0_left, f1_left)
        else:
            selectivity = (f0_right - f0_left) + (f1_right - f1_left)
    ...
    random.shuffle(tmp_matrix)
    pickle.dump(np.array(tmp_matrix, dtype = np.float32), open("biganntest/data/gist_another/gist_query_attrs.pkl", "wb"))
