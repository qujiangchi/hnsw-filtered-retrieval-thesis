...
faiss_include_dir = '/usr/local/include/faiss'
...
if bindings_dir in os.path.basename(os.getcwd()):
    ...
    include_dirs.extend(['../hnswlib/', faiss_include_dir])
else:
    source_files = ['./python_bindings/bindings.cpp']
    include_dirs.extend(['./hnswlib/', faiss_include_dir])


extra_objects = ['/usr/local/include/faiss/IndexACORN.h']
...
setup(
    ...
    package_data={'': ['/usr/local/lib/libfaiss.so']},
